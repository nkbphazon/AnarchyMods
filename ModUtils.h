...
namespace ModUtils
{
	...
	static std::string muGameName = "Anarchy Online";
	static std::string muExpectedWindowName = "Anarchy Online™";
	...
	static bool IsNewEngine()
	{
		return fs::exists("Cheetah.dll");
	}
	...
	static BOOL CALLBACK EnumWindowHandles(HWND hwnd, LPARAM lParam)
	{
		...
		if (processId == GetCurrentProcessId())
		{
			...
			Log("Found window belonging to AO: ", buffer);
